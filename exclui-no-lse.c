...
TLista* exclui(TLista* li, int valor) {
    ...
    TLista* listaAuxiliar = li->prox;
    TLista* ponteiroAnterior = li;
    while(listaAuxiliar != NULL && listaAuxiliar->info != valor){
        listaAuxiliar = listaAuxiliar->prox;
        ponteiroAnterior = ponteiroAnterior->prox;
    }
    if(listaAuxiliar == NULL) return li;
    ponteiroAnterior->prox = listaAuxiliar->prox;
    free(listaAuxiliar);
    return li;
}
